...
#import "ApigeeManager.h"


@implementation ApigeeManager

@synthesize usergridClient;
@synthesize user;


static NSString * prefix = @"[APG]       ";

#pragma mark -
#pragma mark SESSION Init, Dealloc

ApigeeManager * apigeeSelf = nil;				//Context Definition is here!
...
- (id) init {
	...
	apigeeSelf = [self retain];
	if (self) {
		NSString * orgName = APIGEE_ORG_NAME;
		NSString * appName = APIGEE_APP_NAME;
        usergridClient = [[ApigeeClient alloc] initWithOrganizationId:orgName applicationId:appName];
#if 0
		//use ASYNC non-BLOCKING
		if (![usergridClient setDelegate:self]) {
			NSLog(@"%@--- ?Does Not support DELEGATE MODE (%p)",prefix,self);
		}
#else
		//use SYNC BLOCKING
//		[usergridClient setDelegate:nil];
#endif
#if 0
